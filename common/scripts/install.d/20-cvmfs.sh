...
sudo echo -e "# Pull repositories that are in /cvmfs/*.*\n" > $conf
sudo echo -e "CVMFS_REPOSITORIES=\"gm2.opensciencegrid.org\"\n" >> $conf
sudo echo -e "# Talk directly to the stratum 1 server unless overridden.\n" >> $conf
sudo echo -e "CVMFS_HTTP_PROXY=DIRECT\n" >> $conf
sudo echo -e "# Expand quota (units in MB)\n" >> $conf
sudo echo -e "CVMFS_QUOTA_LIMIT=2000\n" >> $conf
unset conf

echo -e "\e[31mAdding gm2.opensciencegrid to cvmfs mounts\e[0m"
sudo cp -r /etc/cvmfs/config.d /etc/cvmfs/backup-config.d
sudo rm -f /etc/cvmfs/config.d/*
sudo echo -e 'CVMFS_SERVER_URL="http://cvmfs-s1bnl.opensciencegrid.org:8000/cvmfs/@fqrn@;http://cvmfs-s1fnal.opensciencegrid.org:8000/cvmfs/@fqrn@;http://cvmfs-s1goc.opensciencegrid.org:8000/cvmfs/@fqrn@"' > /etc/cvmfs/config.d/gm2.opensciencegrid.org.conf
