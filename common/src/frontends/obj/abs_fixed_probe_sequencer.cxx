...
AbsFixedProbeSequencer::AbsFixedProbeSequencer(std::string conf_file, int num_probes) :
  EventManagerBase()
{
...
AbsFixedProbeSequencer::~AbsFixedProbeSequencer()
{
...
int AbsFixedProbeSequencer::Init()
{
  go_time_ = false;
  thread_live_ = false;
...
int AbsFixedProbeSequencer::BeginOfRun()
{
  ...
  // Start threads
  thread_live_ = true;
  run_thread_ = std::thread(&AbsFixedProbeSequencer::RunLoop, this);
  trigger_thread_ = std::thread(&AbsFixedProbeSequencer::TriggerLoop, this);
  builder_thread_ = std::thread(&AbsFixedProbeSequencer::BuilderLoop, this);
  starter_thread_ = std::thread(&AbsFixedProbeSequencer::StarterLoop, this);
...
int AbsFixedProbeSequencer::EndOfRun()
{
  int count = 0;
  do {
    usleep(1000);
...
int AbsFixedProbeSequencer::ResizeEventData(hw::event_data_t &data)
{
  return 0;
}

void AbsFixedProbeSequencer::RunLoop()
{
  while (thread_live_) {
...
void AbsFixedProbeSequencer::TriggerLoop()
{
  while (thread_live_) {
	...
	for (auto &round : trg_seq_) { // {mux_conf_0...mux_conf_n}
	  ...
	  if (generate_software_triggers_) {
	    ...
	    int idx = sis_idx_map_["sis_3302_0"];
...
void AbsFixedProbeSequencer::BuilderLoop()
{
...
void AbsFixedProbeSequencer::StarterLoop()
{
  bool rc = false;
